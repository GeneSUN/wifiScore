...
class extenders_parquet():
    ...
    def __init__(self, 
        ...
        self.device_path = hdfs_pd + "/user/ZheS/wifi_score_v3/deviceScore_dataframe/{}"
    ...
    def get_extender_bef_aft(self, before_extender_date = None, install_extender_date = None, after_extender_date = None, date_window = None):
        ...
        after_install_features = self.agg_device(after_range, df_extender)\
                                .select( "serial_num", col("device_score").alias("target_avg_device_score") )
    ...
    def agg_home(self,  date_range, df_extender, file_path_pattern = None, columns_to_agg = None,  id_columns = None ):   
        if file_path_pattern is None:
            file_path_pattern = hdfs_pd + "/user/ZheS/wifi_score_v3/homeScore_dataframe/{}"
    ...
    def agg_device(self, date_range, df_extender, file_path_pattern = None, columns_to_agg = None,  id_columns = None):
        ...
        if id_columns is None:
            id_columns = ['Rou_Ext', 'avg_phyrate', 'cust_id', 'date', 'dg_model', 'firmware', 'mdn', 'poor_phyrate', 'poor_rssi', 'rk_row_sn', 'rowkey', 'device_score', 'serial_num', 'station_mac', 'stationarity', 'weights']
...
if __name__ == "__main__":
    ...
    #--------------------------------------------------------------------------------
    start_d = date(2024, 2, 13)
    window_range = 1
    #--------------------------------------------------------------------------------
    #for i in range(0,30,window_range):
    d = start_d #+ timedelta(i)
    print(d)
    inst1 = extenders_parquet( sparksession = spark, install_extender_date = d, window_range = window_range)
